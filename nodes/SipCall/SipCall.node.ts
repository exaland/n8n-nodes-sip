...
export class SipCall implements INodeType {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (operation === 'makeCall') {
					...
					body.audioUrl = this.getNodeParameter('audioUrl', i, '') as string;
					body.waitForCompletion = this.getNodeParameter('waitForCompletion', i) as boolean;
					body.timeoutSeconds = this.getNodeParameter('timeoutSeconds', i) as number;
				} else if (operation === 'hangup') {
					endpoint = `/calls/${encodeURIComponent(this.getNodeParameter('callId', i) as string)}/hangup`;
				} else if (operation === 'dtmf') {
					endpoint = `/calls/${encodeURIComponent(this.getNodeParameter('callId', i) as string)}/dtmf`;
					body.digits = this.getNodeParameter('digits', i) as string;
				} else if (operation === 'status') {
					method = 'GET';
					endpoint = `/calls/${encodeURIComponent(this.getNodeParameter('callId', i) as string)}`;
				}
